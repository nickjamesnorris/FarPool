...
export class CreateACarpoolComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.user$.subscribe((user) => {
      if (user != null) {
        ...
        let startingPoint = document.getElementById(
          "startingPoint"
        ) as HTMLInputElement;
        ...
        this.mapsAPILoader.load().then(() => {
          let startingPointAutocomplete = new google.maps.places.Autocomplete(
            startingPoint
          );
  ...
  async onSubmit() {
    ...
    this.flashMessagesService.show("Carpool submitted successfully!", {
